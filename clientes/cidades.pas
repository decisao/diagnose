...
uses data_principal, relaccidades, estados;
 ...
 ######################################################################}
  ...
  ixCampos  : array[1..ixMaximo] of TCampoDes =
     ( (rotulo: 'Código'        ; fisico: 'CODIGO'  ; iTab: 1),
       (rotulo: 'Nome da Cidade'; fisico: 'CIDADE'  ; iTab: 1),
...
procedure TformCidades.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformCidades.btnAdicionarClick(Sender: TObject);
begin
...
procedure TformCidades.FormCreate(Sender: TObject);
begin
     { crio o módulo de dados do relacionamento }
     dmoRelacCidades := TdmoRelacCidades.Create(Self);
...
procedure TformCidades.edtSelecionarKeyPress(Sender: TObject;
  var Key: Char);
var
  i, sel: byte;
  StrSQL: string;
...
procedure TformCidades.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixTabelas[ixCampos[succ(cmbOrdenar.ItemIndex)].iTab] + '.' +
...
procedure TformCidades.cmbSelecionarChange(Sender: TObject);
begin
     edtSelecionar.Clear;
     edtSelecionar.SetFocus;
end;

procedure TformCidades.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformCidades.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformCidades.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformCidades.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformCidades.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformCidades.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformCidades.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
     tag := 0;
...
procedure TformCidades.qryDetalhesAfterInsert(DataSet: TDataSet);
begin
    Dataset.fieldByName(ixCampos[1].fisico).AsInteger :=
      dmoPrincipal.LerSequencia(ixTabelas[1]);
    Dataset.fieldByName('ESTADO').AsString := 'SP';
end;

procedure TformCidades.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformCidades.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
     ...
     with qryDetalhes do
      begin
       ...
       ParamByName(ixCampos[3].fisico).AsString :=
         qrySelecionar.fieldByName(ixCampos[3].fisico).AsString;
       Open;
      end;
...
procedure TformCidades.qryDetalhesAfterOpen(DataSet: TDataSet);
begin
     { mascaras }
     with Dataset do
      begin
       fieldByName('ESTADO').EditMask := 'CC';
       fieldByName('SID').EditMask := '99999';
       fieldByName('DDD').EditMask := '0cc';
...
procedure TformCidades.fldESTADOmudar(Sender: TField);
begin
  if Sender.AsString > '' then
   with dmoRelacCidades.qryEstado do
    begin
     close;
...
procedure TformCidades.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformCidades.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformCidades.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformCidades.lblESTADOClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
...
procedure TformCidades.FormDestroy(Sender: TObject);
begin
     if qrySelecionar.Active then qrySelecionar.Close;
     qryDetalhes.Unprepare;
     dmoRelacCidades.free;
end;


procedure TformCidades.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformCidades.btnAdicionarMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformCidades.btnAdicionarMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
