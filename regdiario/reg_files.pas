...
procedure TformReg_files.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformReg_files.FormShow(Sender: TObject);
var
  i: byte;
begin
...
procedure TformReg_files.FormCreate(Sender: TObject);
begin
...
procedure TformReg_files.edtSelecionarKeyPress(Sender: TObject);
var
  i, sel: byte;
  StrSQL: string;
begin
...
procedure TformReg_files.cmbOrdenarChange(Sender: TObject);
begin
     tmpSQLOrd := ' ORDER BY ' +
       ixCampos[succ(cmbOrdenar.ItemIndex)].fisico;
     with qrySelecionar do
...
procedure TformReg_files.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
  ListItem: TListItem;
...
procedure TformReg_files.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformReg_files.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformReg_files.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformReg_files.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformReg_files.FormDestroy(Sender: TObject);
begin
...
procedure TformReg_files.DBEdit2KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformReg_files.DBEdit1KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformReg_files.DBEdit3KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformReg_files.DBEdit5KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformReg_files.DBEdit6KeyPress(Sender: TObject; var Key: Char);
begin
        dmoPrincipal.PointToComma(Key);

end;

procedure TformReg_files.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformReg_files.ToolbarButton971Click(Sender: TObject);
var
    ...
    begin
     ...
     with Params do
      begin
       ...
       byName('CODREGISTRO').AsInteger := inCliente;
...
procedure TformReg_files.ComboBox1Change(Sender: TObject);
begin
  with ComboBox1 do
    ListView1.ViewStyle := TViewStyle(Items.Objects[ItemIndex]);
end;

procedure TformReg_files.ListView1SelectItem(Sender: TObject;
...
procedure TformReg_files.btnExcluirClick(Sender: TObject);
begin
...
procedure TformReg_files.btnDownloadClick(Sender: TObject);
begin
  if ListView1.Selected = nil then exit;
...
procedure TformReg_files.ListView1Click(Sender: TObject);
begin
...
procedure TformReg_files.ToolbarButton971MouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformReg_files.ToolbarButton971MouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [];
end;

procedure TformReg_files.ApplicationEventsActivate(Sender: TObject);
var
  arquivo: TMemoryStream;
  tamanho_kb: longint;
begin
  if abertura then
   begin
     abertura := false;
     if FileAge(caminho) <> age then
      if Application.MessageBox('O arquivo foi modificado. Deseja atualizar ' +
        'o Banco de Dados?', 'Atualização', mb_YesNo + mb_IconQuestion) = IDYES then
       begin

         { apago o arquivo atual }
         with sqlExcluir do
          begin
         ...
         { pego o tamanho do arquivo }
         tamanho_kb := dmoPrincipal.GetFileSize(caminho).QuadPart;
          ...
          begin
            ...
            with Params do
             begin
              ...
              byName('CODREGISTRO').AsInteger := inCliente;
              byName('NOME').AsString        := ExtractFileName(caminho);
              byName('ORIGINAL').AsString    := caminho;
              byName('DATA').AsDateTime      := dmoPrincipal.GetServerTime;
              byName('USUARIO').AsString     := dmoPrincipal.GetLogUser;
              byName('TAMANHO').AsInteger    := tamanho_kb;
              { carrego o arquivo }
