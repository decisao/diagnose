...
type
  ...
  private
    ...
    { chamadas de rotina }
    procedure fldSETORmudar(Sender: TField);
  public
    { Public declarations }
    inCliente: longint;
...
var
  formExames: TformExames;

implementation

uses data_principal, reconcile, setores;
 ...
 ######################################################################}
  ...
  ixCampos  : array[1..ixMaximo] of TCampoDes =
     ( (rotulo: 'Código'      ; fisico: 'CODIGO'     ; iTab: 1; ),
       ...
       (rotulo: 'Código AMB'  ; fisico: 'CODEXAMB'   ; iTab: 1; ));
...
procedure TformExames.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformExames.btnAdicionarClick(Sender: TObject);
begin
...
procedure TformExames.FormShow(Sender: TObject);
var
  i: byte;
  tecla: Char;
begin
...
procedure TformExames.FormCreate(Sender: TObject);
var
  inI: integer;
begin
...
procedure TformExames.edtSelecionarKeyPress(Sender: TObject;
  var Key: Char);
var
  i, sel: byte;
  StrSQL: string;
...
procedure TformExames.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
...
procedure TformExames.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformExames.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformExames.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformExames.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformExames.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
//     qrySelecionar.tag := 0;
...
procedure TformExames.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformExames.btnExcluirClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
...
procedure TformExames.btnCancelarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformExames.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformExames.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
     CanClose := not cdsDetalhes.Active;
end;

procedure TformExames.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformExames.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformExames.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformExames.btnSalvarClick(Sender: TObject);
const
  myEnter: Char = #13;
begin
     Screen.Cursor := crHourGlass;
...
procedure TformExames.DBEdit6Exit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
       TEdit(Sender).Color := clWindow;
...
procedure TformExames.DBEdit17Enter(Sender: TObject);
begin
     if Sender is TEdit then
      begin
       TEdit(Sender).Color := clInfoBK;
...
procedure TformExames.DBEdit17Exit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
       TEdit(Sender).Color := clWindow;
...
procedure TformExames.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformExames.qrySelecionarAfterScroll(DataSet: TDataSet);
begin
...
procedure TformExames.FormDestroy(Sender: TObject);
var
  inI: integer;
begin
...
procedure TformExames.btnImprimirClick(Sender: TObject);
begin
...
procedure TformExames.btnAdicionarMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformExames.btnAdicionarMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [];
end;

procedure TformExames.btnEmailClick(Sender: TObject);
var
  x: string;
begin
...
procedure TformExames.cdsSelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
...
procedure TformExames.cdsDetalhesNewRecord(DataSet: TDataSet);
begin
...
procedure TformExames.cdsDetalhesAfterOpen(DataSet: TDataSet);
begin

  busca_local := 0;
  with Dataset do
   begin
    fieldByName('SETOR').OnChange := fldSETORmudar;
    fldSETORmudar(fieldByName('SETOR'));
   end;

end;

procedure TformExames.cdsDetalhesAfterPost(DataSet: TDataSet);
begin
...
procedure TformExames.cdsSelecionarAfterRefresh(DataSet: TDataSet);
begin
  with qrySelecionar do
   begin
    if cdsSelecionar.RecordCount > 1 then
...
procedure TformExames.cdsDetalhesAfterDelete(DataSet: TDataSet);
begin
  with (dataset as TClientDataset) do
    ApplyUpdates(1);
end;

procedure TformExames.provDetalhesUpdateData(Sender: TObject;
  DataSet: TClientDataSet);
begin
...
procedure TformExames.cdsDetalhesReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure TformExames.cdsDetalhesBeforePost(DataSet: TDataSet);
begin
  acao := raCorrect;
end;

procedure TformExames.dedNOMEKeyPress(Sender: TObject; var Key: Char);
begin
  if (length(dedNOME.Text) > 23) and (key <> #8) then
    Key := #0;
end;

procedure TformExames.Label8Click(Sender: TObject);
begin
   Screen.Cursor := crHourGlass;
   with TformSetores.Create(Self) do
    try
     ...
     if (ModalResult = mrOk) and (cdsSelecionar.Active) then
       Self.cdsDetalhes.fieldByName('SETOR').AsInteger :=
         cdsSelecionar.fieldByName('CODIGO').AsInteger;
    finally
     release;
    end;
...
procedure TformExames.fldSETORmudar(Sender: TField);
begin
