...
type
  TformTabelaAMB = class(TForm)
    ...
    qryDetalhesNOME: TIBStringField;
    qryDetalhesCALCULO_NOVO: TIBStringField;
    cdsDetalhesCODIGO: TIntegerField;
    cdsDetalhesNOME: TStringField;
    cdsDetalhesCALCULO_NOVO: TStringField;
...
var
  formTabelaAMB: TformTabelaAMB;

implementation

uses data_principal, reconcile, ittabamb;
...
procedure TformTabelaAMB.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformTabelaAMB.btnAdicionarClick(Sender: TObject);
begin
...
procedure TformTabelaAMB.FormShow(Sender: TObject);
var
  i: byte;
  tecla: Char;
begin
...
procedure TformTabelaAMB.FormCreate(Sender: TObject);
var
  inI: integer;
begin
...
procedure TformTabelaAMB.edtSelecionarKeyPress(Sender: TObject;
  var Key: Char);
var
  i, sel: byte;
  StrSQL: string;
...
procedure TformTabelaAMB.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
...
procedure TformTabelaAMB.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformTabelaAMB.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformTabelaAMB.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformTabelaAMB.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformTabelaAMB.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
//     qrySelecionar.tag := 0;
...
procedure TformTabelaAMB.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformTabelaAMB.btnExcluirClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
...
procedure TformTabelaAMB.btnCancelarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformTabelaAMB.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformTabelaAMB.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
     CanClose := not cdsDetalhes.Active;
end;

procedure TformTabelaAMB.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformTabelaAMB.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformTabelaAMB.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformTabelaAMB.btnSalvarClick(Sender: TObject);
const
  myEnter: Char = #13;
begin
     Screen.Cursor := crHourGlass;
...
procedure TformTabelaAMB.DBEdit6Exit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
       TEdit(Sender).Color := clWindow;
...
procedure TformTabelaAMB.DBEdit17Enter(Sender: TObject);
begin
     if Sender is TEdit then
      begin
       TEdit(Sender).Color := clInfoBK;
...
procedure TformTabelaAMB.DBEdit17Exit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
       TEdit(Sender).Color := clWindow;
...
procedure TformTabelaAMB.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformTabelaAMB.qrySelecionarAfterScroll(DataSet: TDataSet);
begin
...
procedure TformTabelaAMB.FormDestroy(Sender: TObject);
var
  inI: integer;
begin
...
procedure TformTabelaAMB.btnImprimirClick(Sender: TObject);
begin
...
procedure TformTabelaAMB.btnAdicionarMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformTabelaAMB.btnAdicionarMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [];
end;

procedure TformTabelaAMB.btnEmailClick(Sender: TObject);
begin
   Screen.Cursor := crHourGlass;
   if cdsSelecionar.Active then
    if not cdsSelecionar.IsEmpty then
     with TformExamesAMB.Create(Self) do
      try
       inTabela := Self.cdsSelecionar.fieldByName('CODIGO').AsInteger;
       SetWhereDynamic(Format('(CODTABAMB = %d)',
        [Self.cdsSelecionar.fieldByName('CODIGO').AsInteger]),
        Format('Tabela AMB %d - %s',
          [Self.cdsSelecionar.fieldByName('CODIGO').AsInteger,
...
procedure TformTabelaAMB.cdsSelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
...
procedure TformTabelaAMB.cdsDetalhesAfterOpen(DataSet: TDataSet);
begin
...
procedure TformTabelaAMB.cdsDetalhesAfterPost(DataSet: TDataSet);
begin
...
procedure TformTabelaAMB.cdsSelecionarAfterRefresh(DataSet: TDataSet);
begin
  with qrySelecionar do
   begin
    if cdsSelecionar.RecordCount > 1 then
...
procedure TformTabelaAMB.cdsDetalhesAfterDelete(DataSet: TDataSet);
begin
  with (dataset as TClientDataset) do
    ApplyUpdates(1);
end;

procedure TformTabelaAMB.provDetalhesUpdateData(Sender: TObject;
  DataSet: TClientDataSet);
begin
...
procedure TformTabelaAMB.cdsDetalhesReconcileError(DataSet: TClientDataSet;
  E: EReconcileError; UpdateKind: TUpdateKind;
  var Action: TReconcileAction);
begin
...
procedure TformTabelaAMB.cdsDetalhesBeforePost(DataSet: TDataSet);
begin
  acao := raCorrect;
end;

procedure TformTabelaAMB.cdsDetalhesNewRecord(DataSet: TDataSet);
begin
  with Dataset do
   begin
     fieldByName(ixCampos[1].fisico).AsInteger :=
       dmoPrincipal.LerSequencia(ixTabelas[1]);
     fieldByName('CALCULO_NOVO').AsString := 'N';
   end;
