...
{######################################################################
 ...
 ######################################################################}
  ...
  ixCampos  : array[1..ixMaximo] of TCampoDes =
     ( (rotulo: 'Código' ; fisico: 'CODPORTE' ; iTab: 1),
...
procedure TformPortes.SetWhereDynamic(st, stmsg: string);
begin
  WhereArgDynamic := st;
  with lblWhere do
   begin
...
procedure TformPortes.btnAdicionarClick(Sender: TObject);
begin
...
procedure TformPortes.FormShow(Sender: TObject);
var
  i: byte;
begin
...
procedure TformPortes.btnSairClick(Sender: TObject);
begin
//     close;
end;

procedure TformPortes.FormCreate(Sender: TObject);
begin
...
procedure TformPortes.edtSelecionarKeyPress(Sender: TObject;
  var Key: Char);
var
  i, sel: byte;
  StrSQL: string;
      ...
      begin
       ...
       with qrySelecionar do
        begin
         ...
         if sel > 0 then StrSQL := StrSQL + ' LIKE ''' +
...
procedure TformPortes.cmbOrdenarChange(Sender: TObject);
begin
     edtPesquisar.Clear;
     tmpSQLOrd := ' ORDER BY ' +
       ixTabelas[ixCampos[succ(cmbOrdenar.ItemIndex)].iTab] + '.' +
...
procedure TformPortes.cmbSelecionarChange(Sender: TObject);
begin
     edtSelecionar.Clear;
     edtSelecionar.SetFocus;
end;

procedure TformPortes.btnImprimirMouseEnter(Sender: TObject);
begin
//     if PrinterMenu.Items.Count = 0 then
//      btnImprimir.Enabled := False;
end;

procedure TformPortes.edtPesquisarChange(Sender: TObject);
begin
...
procedure TformPortes.GridMenuPopup(Sender: TObject);
begin
     AtivarZebra1.Checked := grdDados.Zebra;
end;

procedure TformPortes.Ativarzebra1Click(Sender: TObject);
begin
     grdDados.Zebra := not grdDados.Zebra;
end;

procedure TformPortes.Cordalinha1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformPortes.Cordotexto1Click(Sender: TObject);
begin
     with ColorDialog do
      begin
...
procedure TformPortes.qrySelecionarAfterOpen(DataSet: TDataSet);
var
  i: byte;
begin
     qrySelecionar.tag := 0;
     ...
     for i := 1 to ixMaximo do
      qrySelecionar.fieldByName(ixCampos[i].fisico).DisplayLabel :=
        ixCampos[i].rotulo;
     (qrySelecionar.fieldByName('VALOR') as TNumericField).DisplayFormat := '####0.00';
end;

procedure TformPortes.edtPesquisarKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TformPortes.btnExcluirClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     { salvo o registro }
...
procedure TformPortes.btnCancelarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformPortes.btnModificarClick(Sender: TObject);
begin
     Screen.Cursor := crHourGlass;
     with pgcDados do
      begin
...
procedure TformPortes.FormCloseQuery(Sender: TObject;
  var CanClose: Boolean);
begin
     CanClose := not qryDetalhes.Active;
end;

procedure TformPortes.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  atag: byte;
begin
...
procedure TformPortes.edtPesquisarEnter(Sender: TObject);
begin
...
procedure TformPortes.edtPesquisarExit(Sender: TObject);
begin
     if Sender is TEdit then
      begin
...
procedure TformPortes.btnSalvarClick(Sender: TObject);
const
     myEnter: Char = #13;
begin
     Screen.Cursor := crHourGlass;
...
procedure TformPortes.FormDestroy(Sender: TObject);
begin
     if qrySelecionar.Active then qrySelecionar.Close;
     qryDetalhes.Unprepare;
end;

procedure TformPortes.grdDadosDblClick(Sender: TObject);
var
  scap: string;
begin
  scap := Caption;
...
procedure TformPortes.btnImprimirClick(Sender: TObject);
begin
   with TformGridReport.Create(Self) do
    try
      DBGrid1.DataSource := Self.dsrSelecionar;
      titulo := Self.Caption;
      showmodal;
    finally
      release;
...
procedure TformPortes.btnAdicionarMouseEnter(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
     (Sender as TToolbarButton97).Font.Style := [fsBold, fsUnderline];
end;

procedure TformPortes.btnAdicionarMouseExit(Sender: TObject);
begin
   if Sender is TToolbarButton97 then
